...
const target_block = `                // Empate, mas placar incorreto\r
                map[p.participante_id].pontos += 1;\r
                map[p.participante_id].vencedores += 1; // Podemos considerar como acerto simples na contagem\r
              }\r
            }\r
          }`;
const replacement_block = `                // Empate, mas placar incorreto
...
const user_inputs_block = `                                        onChange={(e) => handleDraftChange(jogo.id, friend.id, 'gols_b', e.target.value)}\r
                                      />\r
                                    </div>\r
                                  ) : (`;
const user_inputs_replacement = `                                        onChange={(e) => handleDraftChange(jogo.id, friend.id, 'gols_b', e.target.value)}
                                      />
                                    </div>
...
const badge_block = `let ptsBadge = null;\r
                            if (isFinished && palpite) {`;
const badge_replacement = `let ptsBadge = null;
                            let pensBadge = null;
                            if (isFinished && palpite) {
...
const pts_badge_render_block = `{ptsBadge && (\r
                                      <div className="absolute -top-2 -right-2 z-10">\r
                                        {ptsBadge}\r
                                      </div>\r
                                    )}\r
...
const pts_badge_render_replacement = `{pensBadge}
                                    {ptsBadge && (
...
// 8. Admin UI
const admin_score_block = `onChange={(e) => handleSaveRealScore(jogo.id, golsRealA !== null ? golsRealA : '', e.target.value)}\r
                                  />\r
                                </>\r
                              ) : (`;
const admin_score_replacement = `onChange={(e) => handleSaveRealScore(jogo.id, golsRealA !== null ? golsRealA : '', e.target.value, jogo.penaltis_vencedor)}
                                  />
...
const admin_finalize_block = `if (isFinished && (\r
      jogo.gols_a === null || \r
      jogo.gols_b === null || \r
      jogo.gols_a === '' || \r
      jogo.gols_b === '' || \r
      ...
      showToast('Preencha um placar válido antes de finalizar.', 'error');\r
      return;\r
    }`;
const admin_finalize_replacement = `if (isFinished && (
      jogo.gols_a === null || 
      jogo.gols_b === null || 
      jogo.gols_a === '' || 
      jogo.gols_b === '' || 
...
// Remove CR because we might be creating mixed line endings if the string doesn't completely match
